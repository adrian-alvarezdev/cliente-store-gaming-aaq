...
export async function getGamesPlatformApi(categorias, limit, start) {
  try {
    const limitItems = `_limit=${limit}`;
    const sortItems = `_sort=createdAt:desc`;
    const startItems = `_start=${start}`;
    const url = `${BASE_PATH}/games?categorias.url=${categorias}&${limitItems}&${sortItems}&${startItems}`;
    const response = await fetch(url);
    const result = await response.json();
    return result;
  } catch (error) {
...
export async function getTotalGamesPlatformApi(categorias) {
  try {
    const url =`${BASE_PATH}/games/count?categorias.url=${categorias}`;
    const response = await fetch(url);
    const result = await response.json();
    return result;
  } catch (error) {
